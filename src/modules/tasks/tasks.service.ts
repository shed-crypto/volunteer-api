...
import {
  ...
  ClearanceLevel, SystemRole, ChatType, OrgRole, TaskOrigin,
} from '@common/enums';
...
export class TasksService {
  ...
  async findByRequest(requestId: string, user: User): Promise<Task[]> {
    const tasks = await this.taskRepository.find({
      where: { requestId },
      relations: ['assignments', 'assignments.user', 'delegations'],
  ...
  async create(requestId: string, dto: CreateTaskDto, creator: User): Promise<Task> {
    ...
    const origin = creator.systemRole === SystemRole.ADMIN
      ? TaskOrigin.ADMIN
      : creator.systemRole === SystemRole.COORDINATOR
        ? TaskOrigin.COORDINATOR
        : TaskOrigin.REQUESTER;
  ...
  async update(id: string, dto: UpdateTaskDto, user: User): Promise<Task> {
    ...
    const isRequestOwner = task.request.creatorId === user.id;
    const isTaskAuthor = task.createdByUserId === user.id;
    const isAdmin = user.systemRole === SystemRole.ADMIN;
    const isCoordinator = user.systemRole === SystemRole.COORDINATOR;

    if (!isTaskAuthor && !isRequestOwner && !isAdmin && !isCoordinator) {
    ...
    const hasActiveAssignments = task.assignments?.some((a) =>
  ...
  async deleteTask(taskId: string, user: User): Promise<void> {
    ...
    const isCoordinator = user.systemRole === SystemRole.COORDINATOR;
    if (!isTaskAuthor && !isRequestOwner && !isAdmin && !isCoordinator) {
      throw new ForbiddenException('Немає прав для видалення цієї підзадачі');
    }

    const hasActiveAssignments = task.assignments?.some(
      (a) => isActiveAssignmentStatus(a.status),
    );
    if (hasActiveAssignments) {
      throw new ForbiddenException('Підзадачу з активними виконавцями не можна видалити. Скасуйте її.');
    }
      ...
      if (task.status === TaskStatus.TODO) {
        ...
        setImmediate(() => {
          const participantIds = [
            request.creatorId,
            ...assignments
              .filter((a) => isActiveAssignmentStatus(a.status))
              .map((a) => a.userId),
            volunteer.id,
          ];
  ...
  async declineDelegation(taskId: string, organizationId: string, user: User): Promise<TaskDelegation> {
    await this.ensureOrgCoordinator(organizationId, user);
    const delegation = await this.delegationRepository.findOne({
      where: { taskId, organizationId },
      relations: ['task'],
    ...
    if (!delegation) throw new NotFoundException('Делегацію не знайдено');
    delegation.isAccepted = false;
    return this.delegationRepository.save(delegation);
  }

  private async ensureOrgCoordinator(organizationId: string, user: User): Promise<void> {
    ...
    if (
      !membership ||
      (membership.orgRole !== OrgRole.LEADER && membership.orgRole !== OrgRole.COORDINATOR)
    ) {
